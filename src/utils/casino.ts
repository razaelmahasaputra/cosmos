...
export async function executeGamble(
    ...
    try {
        const user = await prisma.user.findUnique({ where: { id: jid } });
        ...
        let winAmount = 0;
        let profitChange = BigInt(0);

        if (isWin) {
            winAmount = Math.floor(bet * winMultiplier);
            newBalance += winAmount;
            
            profitChange = BigInt(-1) * BigInt(winAmount - bet); // Bandar loses money
        } else {
            profitChange = BigInt(bet); // Bandar gains money
        }
